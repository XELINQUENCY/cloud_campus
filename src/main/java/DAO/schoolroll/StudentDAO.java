...
public class StudentDAO {
    ...
    // ========== 管理员：更新学生状态（软删除）==========
    int updateStatusById(String studentId, String status) {
    ...
    List<Student> getByMajorId(String majorId) {
        return MyBatisUtil.executeQuery(StudentMapper.class, mapper->mapper.selectByMajorId(majorId));
    }
    List<Student> getByEnrollYear(int year) {
        return MyBatisUtil.executeQuery(StudentMapper.class, mapper->mapper.selectByEnrollYear(year));
    }

    // ========== 组合条件查询 ==========
    List<Student> getByConditions(String studentId, String name, String classId, String majorId, Integer enrollYear, String status) {
    ...
    // ========== 连表查询：学生+班级名+专业名 ==========
    StudentDetailDTO getWithDetailById(String studentId) {
