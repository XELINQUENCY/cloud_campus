...
public class MyProfilePanel extends JPanel {
    ...
    private LibraryProfile libraryProfile; // 保存用户的图书馆档案
    ...
    public MyProfilePanel(User user, IUserClientSrv userSrv) {
        this.currentUser = user;
        this.userSrv = userSrv;
        // 初始化时，libraryProfile 可能为 null，需要通过 refreshData 加载
    ...
    /**
     * 根据 libraryProfile 对象的数据，更新界面显示
     */
    private void updateProfileView() {
        if (libraryProfile != null) {
            double fine = libraryProfile.getFineAmount();

            BigDecimal fineDecimal = BigDecimal.valueOf(fine).setScale(2, BigDecimal.ROUND_HALF_UP);
            ...
            if (fine > 0) {
                ...
                fineAmountLabel.setForeground(new Color(0, 128, 0)); // 深绿色
    ...
    private void refreshData() {
        ...
        new SwingWorker<LibraryProfile, Void>() {
            ...
            protected LibraryProfile doInBackground() throws Exception {
                // 调用服务从服务器获取最新的图书馆档案
                return userSrv.refreshLibraryProfile(currentUser.getId());
            }
            ...
            protected void done() {
                try {
                    ...
                    JOptionPane.showMessageDialog(MyProfilePanel.this, "刷新时发生错误: " + e.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
                } finally {
    ...
    private void handlePayFine() {
        ...
        try {
            ...
            if (amountToPay <= 0) {
                JOptionPane.showMessageDialog(this, "支付金额必须大于0！", "错误", JOptionPane.ERROR_MESSAGE);
                return;
            }
            if (amountToPay > currentFine) {
                JOptionPane.showMessageDialog(this, "支付金额不能超过所欠总额！", "错误", JOptionPane.ERROR_MESSAGE);
                return;
            }
            ...
            new SwingWorker<String, Void>() {
                ...
                protected String doInBackground() throws Exception {
                    // 使用 String 类型的用户ID
                    return userSrv.payFine(currentUser.getId(), amountToPay);
                }
